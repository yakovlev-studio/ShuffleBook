...
import {fotoSpreadTemplates} from '../../../data/FotoSpreadTemplates';
import FotoGalleryContext from '../../../contexts/FotoGallery';
import {viewportMiddlePoint} from './styles';
...
const FotoSpread = ({_, index}) => {
  ...
  const photoSpreads = useSelector(
    ({shuffleBook}) => shuffleBook['photoSpreads'],
  );
  console.log(photoSpreads);
  const activePhotoSpread = useSelector(
    ({shuffleBook}) => shuffleBook.activePhotoSpread,
  );
  ...
  return (
    <View style={[styles.fotoSpread]}>
      <View style={styles.fotoSpreadInnerContainer}>
        ...
        {fotoSpreadTemplates.get(templateIndex).fotoSpread.get('meta')
          .isDashRequired && (
