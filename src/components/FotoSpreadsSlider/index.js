...
import FotoSpread from './FotoSpread';
import {sliderHeight, slideWidth, itemHeight} from './FotoSpread/styles';
...
const FotoSpreadsSlider = () => {
  ...
  const photoSpreads = useSelector(({shuffleBook}) => {
    // console.log('inside useSelector');
    // console.log(shuffleBook['photoSpreads'].length);
    return shuffleBook['photoSpreads'];
  });
  const leftEdgeOfDroppedPhoto = useSelector(
    ({shuffleBook}) => shuffleBook.leftEdgeOfDroppedPhoto,
  );

  const ENTRIES = Array.from({length: 5}).map(_ => ({
    id: v4(),
  }));
