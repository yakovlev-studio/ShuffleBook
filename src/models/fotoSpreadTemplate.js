import {
  ...
  slideWidth,
  slideHeight,
} from '../components/FotoSpreadsSlider/FotoSpread/styles';
...
export class FotoSpreadTemplate {
  ...
  _generateStyles(props) {
    ...
    props.forEach((mapObj, index) => {
      ...
      var outerFotoFramesContainerProps = mapObj.get(
        'outerFotoFramesContainer',
      );
      var {rows, cols, direction = undefined} = mapObj.get(
      ...
      if (
        cols === 1 &&
        rows === 1 &&
        style.outerFotoFramesContainer.padding === 0
      ) {
        borderTopWidth = 0;
        borderRightWidth = 0;
        borderBottomWidth = 0;
      ...
      fotoFramesProps.forEach((frame, index) => {
        ...
        fotoFrames.set(index + 1, {
          width:
            '' +
            (widthOfFotoFrame -
              calculatePercentOfTotal(horizontalMargin, slideWidth)) +
            '%',
          height:
            '' +
            (heightOfFotoFrame -
              calculatePercentOfTotal(verticalMargin, slideWidth)) +
            '%',
    ...
    return this._computeTracksCoordinates(
      this._mapCellsToPhotoFramesIndices(styles),
    );
  }
  ...
  _computeTrackCoordinates({styles}, side) {
    ...
    var sizeOfSingleRow =
      heightOfFotoFramesContainer / fotoFramesContainer.meta.layout.rows;
    var sizeOfSingleCol =
      widthOfFotoFramesContainer / fotoFramesContainer.meta.layout.cols;
    ...
    var computeTracksCoodinates = function(sizeOfSingleTrack, tracks) {
      ...
      for (
        let track = 0;
        track < fotoFramesContainer.meta.layout[tracks];
        track++
      ) {
